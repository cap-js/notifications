...
const { getNotificationDestination, doesKeyExist } = require("./utils");

const NOTIFICATION_TYPES_API_ENDPOINT = "v2/NotificationType.svc";

function createNotificationTypesMap(notificationTypesJSON, prefix) {
  const types = {};
  notificationTypesJSON.forEach((notificationType) => {
    const key = `${prefix}/${notificationType.NotificationTypeKey}`;
    if (!doesKeyExist(types, key)) {
      types[key] = {};
    }

    types[key][notificationType.NotificationTypeVersion] = notificationType;
  });
...
async function processNotificationTypes(notificationTypes, prefix, destination) {
  ...
  for(let notificationTypeKey in notificationTypes) {
    for(let notificationTypeVersion in notificationTypes[notificationTypeKey]) {
      createNotificationType(notificationTypes[notificationTypeKey][notificationTypeVersion]);
    }
  }
}
