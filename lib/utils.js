...
function replaceRefsInExpr(expr, data) {
  ...
  if ('ref' in expr && !expr.param) {
    if (expr.ref.length === 1) {
      ...
      if (key in data) return { val: data[key], param: false }
    }
    return expr
  }
...
async function evaluateDynamicPriority(xpr, data) {
  ...
  try {
    ...
    LOG._warn && LOG.warn('Dynamic priority expression could not be evaluated, falling back to NEUTRAL')
    return undefined
  }
  ...
  if (!validatePriority(priority)) return undefined
  return priority
}
