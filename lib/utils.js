...
async function evaluateDynamicPriority(xpr, data) {
  ...
  const anyEntity = Object.values(cds.model.definitions).find(d => d.kind === 'entity')
  if (!anyEntity) return undefined
  const col = { ...resolvedXpr, as: 'result' }
  const result = await SELECT.one.from(anyEntity.name).columns(col)
  if (!result) {
...
async function buildNotificationFromEvent(eventDef, data = {}) {
  ...
  if (priorityAnnotation) {
    if (priorityAnnotation.xpr) {
      Priority = await evaluateDynamicPriority(priorityAnnotation, data) ?? 'NEUTRAL'
    } else {
