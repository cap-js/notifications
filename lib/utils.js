...
const messages = {
  ...
  PROPERTY_VALUE_TOO_LONG: "Property value exceeds the maximum length of 255 characters.",
}
...
function applyValueLengthConstraints(notification) {
  ...
  if (notification.Properties) {
    for (const prop of notification.Properties) {
      if (String(prop.Value ?? '').length > 255) {
  ...
  if (notification.TargetParameters) {
    notification.TargetParameters = notification.TargetParameters.filter(
      param => String(param.Value ?? '').length <= 250
    )
  }
