  ...
  for (const def of model.definitions) {
    ...
    if (def['@notification.channels']?.length) {
      type.DeliveryChannels = def['@notification.channels'].map(ch => {
        const isObject = ch && typeof ch === 'object' && !('=' in ch) && !('#' in ch)
        const name = isObject ? resolveEnum(ch.channel) : resolveEnum(ch)
        if (typeof name !== 'string') return null
        const channelType = CHANNEL_MAP[name.toLowerCase()] ?? name.toUpperCase()
        return {
          Type:              channelType,
          Enabled:           isObject && ch.enabled            !== undefined ? ch.enabled            : true,
          DefaultPreference: isObject && ch.defaultPreference  !== undefined ? ch.defaultPreference  : true,
          EditablePreference: isObject && ch.editablePreference !== undefined ? ch.editablePreference : true,
        }
      }).filter(Boolean)
    }
